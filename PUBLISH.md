...
**错误 403 - You cannot publish over the previously published versions: X.X.X**:
- ✅ 这是版本冲突错误，表示该版本已经在 npm 上存在
- ✅ 工作流已自动处理：如果检测到版本冲突，会自动再次递增版本号
- ✅ 如果自动递增后仍然冲突，工作流会失败并提示手动更新版本号
- 💡 **解决方案**：手动更新 `package.json` 中的版本号为一个未发布的版本，然后重新运行工作流
...
4. 选择版本类型：
   ...
   - `minor`: 次要版本（0.1.0 -> 0.2.0）
   - `major`: 主要版本（0.1.0 -> 1.0.0）
5. 点击 "Run workflow" 按钮

工作流会自动：
- 检查并更新版本号（如果版本已存在会自动递增）
- 运行测试
- 构建项目
- 验证版本号可用性
- 发布到 npm
...
5. **Bump version** - 更新版本号（先更新版本，确保构建时使用新版本号）
   - 自动检查 npm 上是否已存在该版本
   - 如果版本已存在，会自动再次递增版本号
6. **Verify version** - 验证版本号可用性（确保版本不存在于 npm）
7. **Build** - 运行 `npm run build` 构建项目（使用新版本号）
8. **Run tests** - 运行 `npm test` 执行测试
9. **Publish to npm** - 发布到 npm
10. **Create Git tag** - 创建 Git 标签（仅 workflow_dispatch）
11. **Create GitHub Release** - 创建 GitHub Release（仅 workflow_dispatch）
...
1. **版本号**: 
   - 确保版本号遵循语义化版本（Semantic Versioning）
   - 工作流会自动检查版本冲突，如果版本已存在会自动递增
   - 如果遇到版本冲突，可以手动更新 `package.json` 中的版本号
2. **测试**: 发布前确保所有测试通过
3. **构建**: 确保 `npm run build` 能成功生成 `ppt.js`
4. **文件**: 检查 `.npmignore` 确保不需要的文件不会被发布
5. **权限**: 确保 NPM_TOKEN 有发布权限
6. **版本检查**: 工作流会在发布前验证版本号是否可用，避免重复发布
